        ...
        <form action="{{route('timkiemPCHT')}}" class="adminFormSearch" method = "get">
          <table class="adminFormSeachTable">
            ...
            <tr>
              ...
              <td>
                  <input type="text" name = "MaHS" value="{{Request::get('MaHS')}}" class = "formInput"  placeholder="Mã học sinh">
              </td>
            </tr>
            <tr>
              <td> <p class="adminFormSearchText">Tên học sinh</p></td>
              <td>
                  <input type="text" name = "TenHS" value="{{Request::get('TenHS')}}" class = "formInput" placeholder="Tên học sinh">
              </td>
            </tr>
          </table>
          <div class="adminFormAdd">
            <div class="adminFormAddGroup adminFormSearchContainsBtn">
                <button type="submmit" class="adminFormSearchBtn d-inline-block">Tìm kiếm</button>
                <a href="admin/hocchonkhoivalop" class="adminFormAddLink">Làm mới</a>
            </div>
          </div>
        </form>
      </div>
      ...
      <div class="container">
          ...
          <table class="adminTable" border="1">
              ...
              @foreach ($pcht as $item)
                  <tr>
                      <td><input type="checkbox" class="checkBoxStudent" value="{{$item->idPCHT}}" name="checkBoxGetId[]"></td>
                      <td>{{$stt++}}</td>
                      <td>{{$item->MaHS}}</td>
                      ...
                      <td>
                        @foreach ($nienkhoa as $nk)
                          @if ($nk->id == $item->id_nienkhoa)
                            {{$nk->NamBatDau . ' - ' . $nk->NamKetThuc}}
                          @endif
                        @endforeach
                      </td>
                      <td>
                        @if ($item->LenLop === 0) Ở lại lớp @endif
                        @if ($item->LenLop === 1) Lên lớp @endif
                        @if ($item->LenLop === NULL) Chưa đánh giá @endif
                      </td>
                      <td>
                          <a href="admin/hoc/{{$item->idPCHT}}/edit"><i class="fas fa-edit"></i></a>
                          <form action="{{route('hoc.destroy','')}}/{{$item->idPCHT}}" method = "post" class="adminFormAdd {{'formDelete' . $item->idPCHT}} d-inline" >
                              @method('DELETE') @csrf
                              <button type="button" class="bg-none border-0 btnButton" id="{{$item->idPCHT}}" data-toggle="modal" data-target="#exampleModal">
